    ...
    <div className={getScreenClass("s-onboarding")} id="s-onboarding">
      ...
      >
        ...
        <button
          ...
          onClick={() => showToast("Google 로그인 화면으로 이동합니다")}
        >
        ...
        <div className="form-note">
          국적·학교·비자 유형만 입력하면 바로 시작할 수 있어요
        </div>
        <div className="form-group" style={{ marginTop: "8px" }}>
          <div className="f-label">국적</div>
          <div className="f-input filled">
            🇨🇳 중국 <span style={{ color: "var(--c-t3)" }}>▾</span>
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <div className="chip-row">
            {["D-2 학생", "D-4 어학연수", "F-2 거주", "기타"].map((v) => (
              <button
                key={v}
                className={`sel-chip${visaChip === v ? " on" : ""}`}
                onClick={() => setVisaChip(v)}
              >
                {v}
              </button>
            ))}
        ...
        <div className="ob-note">
          📅 비자 만료일은 비자 채널에서 대화할 때 입력할 수 있어요
        </div>
        ...
        <div className="lang-grid">
          {[
            ...
            ["en", "🇺🇸", "영어", "English"],
            ["vi", "🇻🇳", "베트남어", "Vietnamese"],
          ].map(([key, flag, name, sub]) => (
            <div
              key={key}
              className={`lang-card${langs[key] ? " on" : ""}`}
              onClick={() => setLangs((l) => ({ ...l, [key]: !l[key] }))}
            >
        ...
        <button className="cta-primary" onClick={() => navigate("s-home")}>
          시작하기 →
        </button>
        <div className="footnote">
          국적·학교·비자 유형만으로 맞춤 채널이 자동 생성됩니다
        </div>
      </div>
    </div>
  );
